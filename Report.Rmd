...
- The dataset contains 4987 observations that corresponds to observations measured at every 15 minutes. 
- Also, we can see the presence of 96 missing values for consumption that need to be further investigated.


### Missing Values {#missing-values}

We can see that the 96 missing values for consumption are the ones to be predicted by our best model at the end, so the missing values do not need to be imputed.
...
```{r}
plot(ts[,"consumption"], main = "Consumption Time-Series")
...
Obervations with zero consumption:

We can see we have very few observations that have consumption zero that have a time frame of approximately 2 hours. It could indicate a blackout during that period or problem with the data collection. For these points, the lower average bigger than zero will be attributed.
```{r}
...
Time series after removing initial outliers
```{r}
...
plot(ts[,"consumption"], main = "Consumption Time-Series wihtout zeros")
```
...
# Encoding Weekday features to test it as covariate
data[,"weekday0"] = 0
data[,"weekday1"] = 0
data[,"weekday2"] = 0
...
```
Demand period analysis (Low, Medium and High demand)
...
```{r}
mean_consumption = data %>% group_by(format(time, "%H:%M:%S")) %>%  summarise(mean_consump = mean(consumption, na.rm = TRUE))
colnames(mean_consumption) = c("time","mean")

transitions = list("08:00:00", "08:15:00","17:00:00","17:15:00", "23:00:00", "23:15:00")
...
Creating "demand" feature to represent this changes in demand
...
# 3) Replace those days with the third quantile for all the days if higher value outliers

# For each time period of the day (regardless of the day of the week at this point) calculate the respective mean and the respective 3rd quantile.
...
# Preparing XREG covariates that will be used for testing
```{r}
...
- Initially, to select the best model type, the time series will be splitted into train and test set.
...
```
The resulting train sample seems to be representative of the previous plots.

## Modeling {#modeling}

The best model selection was divided into two parts: without using temperature covariate and with the temperature covariate as part of the model training.
For each model trained, the error is collected for comparison at the end.
...
Support functions used with different models
```{r}
...
LITTLE TEXT ABOUT THE BEST MODELA
